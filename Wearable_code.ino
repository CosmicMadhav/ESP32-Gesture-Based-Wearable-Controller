...
BluetoothSerial BT;                     // Bluetooth Serial instance
AccelAndGyro Ag;                         // MPU6050 interface
LightProximityAndGesture Lpg;            // APDS9960 Gesture Sensor interface
oLed display(SCREEN_WIDTH, SCREEN_HEIGHT); // OLED display object

/* Sensor Data Variables */
float AngleRoll = 0, AnglePitch = 0;     // Current tilt angles
float RefAngleRoll = 0, RefAnglePitch = 0; // Reference angles for calibration
float filteredRoll = 0, filteredPitch = 0; // Filtered angles for stability

/* Device Control Variables */
int currentDevice = 0;                   // Currently selected device index
bool deviceSelected = false;             // Flag to check if a device is selected
unsigned long lastSwitchTime = 0;        // Last time device was switched
const int SWITCH_DEBOUNCE = 500;         // Debounce time for switching devices (ms)
unsigned long lastMovement = 0;          // Last detected movement time
...
const float DEADBAND = 5.0;  // Added declaration
...
void checkGestures() {
  ...
  if (deviceSelected) {
    ...
    switch (gesture) {
      ...
      case SWIPE_RIGHT:
        deviceSelected = false;
        Serial.println(" Swipe Left → Exit");
